...
namespace DoctorFAM.Data.Repository
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<List<string>?> GetAdminsAndSupportersNotificationForSendNotificationInHomePharmacy()
        {
            ...
            //Get Admins User Id
            var admins = await _context.Users.AsNoTracking().Where(p => !p.IsDelete && p.IsAdmin).Select(p => p.Id.ToString()).ToListAsync();
            model.AddRange(admins);

            //Get Supporters User Id
            var supporters = await _context.UserRoles.AsNoTracking().Where(p => !p.IsDelete && p.RoleId == 9).Select(p => p.UserId.ToString()).ToListAsync();
        ...
        public async Task<List<string>?> GetAdminsAndSupportersNotificationForSendNotificationInHomeLaboratory()
        {
            ...
            //Get Supporters User Id
            var supporters = await _context.UserRoles.Where(p => !p.IsDelete).Select(p => p.UserId.ToString()).ToListAsync();
            model.AddRange(supporters);
