...
{
    public class ProductController : MarketBaseController
    {
        #region Ctor
        ...
        public async Task<IActionResult> FilterProducts(FilterProductMarketSideViewModel filter)
        {
        ...
        public async Task<IActionResult> CreateProduct()
        {
            ...
            ViewData["FirstCategory"] = await _productService.GetAllFirstProductCategories();
        ...
        public async Task<IActionResult> CreateProduct(CreateProductsViewModel model, IFormFile? ProductImage)
        {
            #region Page Data 
            ...
            var result = await _productService.CreateProductFromSellerPanel(model, ProductImage);

            switch (result)
            {
                case CreateProductSellerPanelResult.Success:
                    TempData[SuccessMessage] = "عملیات با موفقیت انجام شده است .";
                    return RedirectToAction(nameof(FilterProducts));

                case CreateProductSellerPanelResult.ImageName:
                    TempData[ErrorMessage] = "تصویر محصول باید وارد شود .";
                    break;

                case CreateProductSellerPanelResult.LongDescription:
                    TempData[ErrorMessage] = "توضیح کامل محصول باید وارد شود .";
                    break;

                case CreateProductSellerPanelResult.UserNotFound:
                    TempData[ErrorMessage] = "کابر موردئ نظر یافت نشده است .";
                    break;

                case CreateProductSellerPanelResult.Categories:
                    TempData[ErrorMessage] = "دسته بندی های محصول یافت نشده است .";
                    break;

                case CreateProductSellerPanelResult.Faild:
                    TempData[ErrorMessage] = "عملیات با موفقیت شکست مواجه شده است . .";
                    break;

                case CreateProductSellerPanelResult.PackedNote:
        ...
        public async Task<IActionResult> EditProduct(ulong id)
        {
            ...
            var model = await _productService.FillEditProductInSellerPanelViewModel(id , User.GetUserId());
        ...
        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> EditProduct(EditProductViewModel model , IFormFile? ProductImage)
        {
            #region Get Product

            var product = await _productService.FillEditProductInSellerPanelViewModel(model.ProductId, User.GetUserId());
            ...
            var result = await _productService.EditProductFromSellerPanel(model, ProductImage);
