...
{
    public class TicketController : SupporterBaseController
    {
        #region Ctor
        ...
        public async Task<IActionResult> CreateTicket(AddTicketViewModel ticket)
        {
            ...
            var res = await _ticketService.AddTicketFromSupporterPanel(ticket, User.GetUserId());

            if (res)
            {
                TempData[SuccessMessage] = _localizer["mission accomplished"].Value;
                return RedirectToAction("FilterTickets", "Ticket", new { area = "Supporter" });
            }
        ...
        public async Task<IActionResult> TicketDetail(AnswerTicketAdminViewModel answer)
        {
            ...
            {
                TempData[SuccessMessage] = _localizer["mission accomplished"].Value;
                return RedirectToAction("TicketDetail", "Ticket", new { area = "Supporter", id = answer.TicketId });
            }
