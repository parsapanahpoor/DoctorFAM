...
{
    public class TicketController : PharmacyBaseController
    {
        #region Ctor
        ...
        public async Task<IActionResult> CreateTicket(CreateTicketViewModel create)
        {
            ...
            var result = await _ticketService.CreateTicket(create, organization.OwnerId, Domain.Enums.Ticket.TicketSenderType.FromPhamracy);

            if (result != 0)
            {
                TempData[SuccessMessage] = _localizer["mission accomplished"].Value;
                return RedirectToAction("TicketDetail", "Ticket", new { area = "Pharmacy", id = result });
            }
        ...
        public async Task<IActionResult> TicketDetail(AnswerTicketViewModel answer)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    TempData[SuccessMessage] = _localizer["mission accomplished"].Value;
                    return RedirectToAction("TicketDetail", "Ticket", new { area = "Pharmacy", id = answer.TicketId });
                }
            }
